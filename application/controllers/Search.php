...
class Search extends CI_Controller {
	...
	public function index()
	{
       ...
       $data['instruction'] = $this->appointment_instruction_model->read_active_instuction();
		   ...
		   $data['all_doctors'] = $this->doctor_model->doctor_list();
			 $data['all_doctors_spec'] = $this->doctor_model->read_doctor_language();

		//	$data['doctor_list'] = $this->doctor_model->doctor_list();
		// 	echo "<pre>";
		// 	var_dump($data['all_doctors']);
		//  echo "</pre>";
		//	$this->db->query("SELECT * FROM `departements` where 'active' =1 ");

	  	$data['content'] = $this->load->view('website/search',$data,true);
